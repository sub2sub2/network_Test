...
if [[ "$BUILD_SIMPLE" == false && "$BUILD_ADVANCED" == false ]]; then
    BUILD_SIMPLE=true
    BUILD_ADVANCED=true
fi
...
if ! command -v clang++ &> /dev/null; then
    print_error "clang++ 컴파일러를 찾을 수 없습니다."
    exit 1
fi
...
if [[ "$CLEAN" == true ]]; then
    ...
    rm -f curl_cpp_simple advanced_curl_cpp
    print_success "정리 완료"
    exit 0
fi
...
if [[ "$BUILD_SIMPLE" == true ]]; then
    ...
    if clang++ $COMPILE_FLAGS -o curl_cpp_simple curl_cpp_simple.cpp -lcurl; then
...
if [[ "$BUILD_ADVANCED" == true ]]; then
    ...
    if clang++ $COMPILE_FLAGS -o advanced_curl_cpp advanced_curl_cpp.cpp -lcurl; then
